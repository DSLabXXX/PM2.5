...
"""
Rollout is important for value estimation because it "looks ahead" to see 
how future actions might follow from whatever action you take.  
This prevents short-term gain from being exploited when follow actions are particularly bad.
"""
...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        ...
        def _g_recurrence_1(i, x_t, h_tm1, given_num, gen_x):
            # Def:
            #   LSTM forward operation unit, inputs from 0 to t are known.
            #   This function is used prediction time slice from 0 to t.
            # Args ------------
            #   i: counter
            #   x_t: input at time t
            #   h_tm1: a tensor that packs [prev_hidden_state, prev_c], i.e., h_{t-1}
            #   given_num: t
            #   gen_x: list to record output sequence
            # Returns ------------
            #   i + 1: next counter
            ...
            #   given_num: t
            #   gen_x: add next_token to the list, i.e., to record each predicted input from 0 to t
        ...
        # self.gen_x denotes input for all the sequence.
        # [0, t] is given, [(t+1), T] is predicted.
        i, x_t, h_tm1, given_num, self.gen_x = control_flow_ops.while_loop(cond=lambda i, _1, _2, given_num, _4: i < given_num,
                                                                           body=_g_recurrence_1,
                                                                           loop_vars=(tf.constant(0, dtype=tf.int32),
                                                                                      tf.nn.embedding_lookup(self.g_embeddings, self.start_token),
                                                                                      self.h0, self.given_num, gen_x))

        _, _, _, _, self.gen_x = control_flow_ops.while_loop(cond=lambda i, _1, _2, _3, _4: i < self.sequence_length,
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator, sequence_length):
        ...
        # Args ------------
        #   sess: tf.Session()
        #   input_x: given input i.e., [0, t]
        #   rollout_num: the number of rollout, e.g., (T-t)
        #   discriminator:
        # Returns ------------
        #   rewards:
        rewards = [0.] * sequence_length
        for i in range(rollout_num):  # for n = 1 to N
            for given_num in range(1, sequence_length):  # for t = 1 to (T-1)
